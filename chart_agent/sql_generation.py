...
# ==================== 工具函数 ====================
def _load_er_image() -> dict:
    """加载 ER 图为 Gemini inline_data 格式"""
...
def run_sql_generation(
    ...
    model_pro = genai.GenerativeModel("gemini-2.5-pro-preview-03-25")
    ...
    for attempt in range(1, MAX_RETRY + 1):
        ...
        # ER 图
        prompt_parts.append({"inline_data": er_image})
        # 用户当前输入
        prompt_parts.append(
            f"用户问题：{user_input}\n\n"
        ...
        try:
            sql_response = model_pro.generate_content(
                prompt_parts,
        ...
        validation_parts.append({"inline_data": er_image})
        ...
        try:
            val_response = model_pro.generate_content(
                validation_parts,
                generation_config=genai.GenerationConfig(
                    response_mime_type="application/json",
                    response_schema={
